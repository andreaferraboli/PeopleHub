...
class ObserveAllTagsUseCase @Inject constructor(
    private val repository: PeopleRepository,
) {
 ...
 */
class UpsertPersonUseCase @Inject constructor(
    private val repository: PeopleRepository,
) {
    suspend operator fun invoke(person: Person): Result<Long> = runCatching {
        require(person.firstName.isNotBlank()) { "A name is required" }
        repository.upsertPerson(
            person.copy(
                firstName = person.firstName.trim(),
                lastName = person.lastName.trim(),
                tags = person.tags.map(String::trim).filter(String::isNotBlank).distinct(),
 ...
 */
class DeleteAllPeopleUseCase @Inject constructor(
    private val repository: PeopleRepository,
) {
